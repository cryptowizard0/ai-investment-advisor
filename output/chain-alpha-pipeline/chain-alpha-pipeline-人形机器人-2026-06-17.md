...
- Step 3 负控/背景验证: output/chain-alpha-verification/chain-alpha-verification-603667-2026-06-17.md
- Step 3: output/chain-alpha-verification/chain-alpha-verification-MP-2026-06-17.md

## 二、一句话结论

Proof rerun 成功验证：**A 股候选可以进入 Step 3 并获得真实档位/仓位结论，不能仅因非美上市地被剔除**。绿的谐波（688017.SH）进入并获得通过档；金力永磁（300748.SZ）进入但因机器人专项收入占比不足为待验证；五洲新春（603667.SH）因 PRS/机器人收入占比约 0.22% 低于 20% Step 2 硬门槛，不计入正常 Step 3 漏斗，仅保留负控/背景验证卡；MP 作为美股 comparator 维持待验证。

## 三、错位环节与评分

| 环节 | 错位评分 | 本次处理 |
|---|---:|---|
| 行星滚柱丝杠 PRS | 20/20 | Step 2 保持收入占比硬门槛；五洲新春硬门槛失败，仅作负控/背景验证，不计入正常 Step 3 漏斗 |
...
| **可投候选进入验证（标注市场）** | **3** | 688017.SH（A 股科创板）、300748.SZ（A 股创业板）、MP（NYSE） |
| 负控样本/背景验证（不计入漏斗） | 1 | 603667.SH（A 股主板）：Step 2 PRS/机器人收入占比约 0.22%，低于 20% 硬门槛；验证卡仅证明剔除原因不是上市地 |
...
### 负控/背景验证（不计入正常 Step 3 漏斗）

| 公司 | Ticker | 市场状态 | 处理 | 结论 |
|---|---|---|---|---|
| 五洲新春 | 603667.SH | A 股上交所主板 | 负控样本（不计入正常 Step 3 漏斗） | PRS/机器人收入约 0.22%，低于 20% 硬门槛；拒绝原因是收入占比硬门槛，不是 A 股上市地；无仓位 |
...
| 旧行为风险 | 本次 proof 结果 |
|---|---|
| Step 2 使用 `US-listed/ADR 候选` 语义，把 A 股强标的归为背景 | 稀土永磁与谐波减速器 Step 2 报告使用 `进入第三步的可投候选（标注市场）` 与 `上市地与可投性` 字段；PRS 报告保留该章节但明确无正常候选，并将 603667.SH 移入负控/背景样本 |
| A 股候选被市场属性提前剔除 | 688017.SH、300748.SZ 正常进入 Step 3；603667.SH 因 Step 2 收入占比硬门槛失败，仅保留负控验证卡，不计入正常漏斗 |
| 非市场原因没有被清晰区分 | 603667.SH 被收入占比硬门槛剔除；300748.SZ 因机器人专项收入占比待验证；二者都不是因 A 股上市地剔除 |

...
开始时探测当前 agent 会话是否支持且允许派发 subagent（并行子任务）：

- **Claude Code**：若平台支持 Task/subagent 派发，进入并行模式，Step 2 按环节、Step 3 按公司 fan-out。
- **Codex**：仅当用户明确要求 subagent、delegation 或 parallel agent work，且当前会话暴露 subagent 派发工具（如 `spawn_agent` / `wait_agent` / `close_agent`）时，进入并行模式。
- **未授权、工具不可用或探测失败**：降级为串行模式，在当前会话内顺序处理每个环节/公司。

并行与串行两种模式的硬门槛、漏斗纪律、交接字段、报告格式完全一致；唯一区别是是否并行。降级是默认安全路径：任何未授权、探测失败或派发不可用，都回到串行模式，保证 Codex 与 Claude Code 的报告可比。
...
- **Codex 并行约束**：派发前必须满足用户显式授权；每个 `spawn_agent` 任务使用自包含 prompt；主 agent 用 `wait_agent` 收齐 final message 交接字段，完成后关闭或释放已完成 subagent。
...
- **未授权、能力探测失败或并行不可用**：整体降级为串行模式，行为与产出格式与并行模式一致。

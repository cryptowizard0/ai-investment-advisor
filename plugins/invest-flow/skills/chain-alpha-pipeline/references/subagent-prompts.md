...
Codex 只有在用户明确要求 subagent、delegation 或 parallel agent work，且当前会话暴露 subagent 派发工具时，才启用本文件的并行派发模板；否则直接使用串行降级模式。
...
report_path: <报告路径>
candidates:
- 公司 | Ticker | 子环节 | 候选25分 | 占比初值(置信度) | CR3证据等级 | 是否美股或ADR(是/否/仅粉单) | 待核实事项
若该环节 0 家通过硬门槛：注明"格局分散，暂不可投"并列格局最好的公司作背景。
```

## Step 2 → Step 3 主 agent 收口

主 agent 收齐各环节候选后，按候选 25 分排序，只挑美股/ADR 候选 **top-K（默认 6）** 进入 Step 3 fan-out，不对全量候选逐一派发。单波并行默认 ≤6 个 subagent，超出分批。
...
若当前 agent 会话未获用户授权、工具不可用或派发失败，主 agent 不使用本文件的派发动作，改为在当前会话内对每个环节/公司顺序套用上述同样的任务说明与交接字段，产出格式完全一致。

...
description: "多Agent协同股票分析系统 - 在当前 agent 会话（Codex / Claude Code）中按 prompt 编排公司画像、基本面、机构资金、反身性、Reportify 和非共识分析，并聚合为中文综合投资判断。适用于：(1) 需要多维度验证的投资决策, (2) 寻找高置信度交易机会, (3) 全面评估标的投资价值。"
---
...
本 skill 的当前真实入口是 agent 会话（Codex / Claude Code）内的 prompt 编排：解析用户给出的 ticker/company，依次调用六个 InvestFlow 子 skill，要求每个子 skill 先保存 Markdown 子报告并返回 `report_path`，再收集公司画像、结论、证据、风险、置信度、数据缺口和非共识变量，最后输出中文综合报告。
...
它不会执行六个子 skill。真实分析仍由当前 agent 会话按上面的六段 prompt 完成。
...
综合报告保存前必须检查六个维度的 `report_path`。若某个子维度没有路径，先补跑该子 skill；只有补跑失败或用户明确允许部分结果时，才允许在综合报告中写入“未生成子报告链接（failed/partial）”。

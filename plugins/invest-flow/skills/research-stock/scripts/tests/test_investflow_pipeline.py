...
class ModelTests(unittest.TestCase):
    def test_stage_result_to_dict_contains_prompt_and_handoff(self):
        ...
        result = StageResult(
            ...
            prompt="使用 invest-flow:research-fundamentals 分析 TSLA",
        ...
        self.assertEqual(data["prompt"], "使用 invest-flow:research-fundamentals 分析 TSLA")
    ...
    def test_pipeline_result_to_dict_counts_success_failed_and_pending(self):
        ...
        result = PipelineResult(
            ...
            stage_results=[
                StageResult("research-fundamentals", "fundamental", AnalysisStatus.SUCCESS),
                StageResult("research-reportify", "reportify", AnalysisStatus.FAILED),
                StageResult(
...
class RegistryTests(unittest.TestCase):
    ...
    def test_company_profile_prompt_captures_business_context(self):
        ...
        spec = build_registry().get("research-profile")
    ...
    def test_earnings_is_available_but_not_in_default_stock_plan(self):
        ...
        registry = build_registry()
        spec = registry.get("research-earnings")
...
class PlannerTests(unittest.TestCase):
    ...
    def test_basic_plan_uses_five_prompt_specs_with_company_profile_first(self):
...
## 核心结论
公司画像背景可作为辅助阅读，但本报告不是 research-profile 模板。
...
class ComposerTests(unittest.TestCase):
    ...
    def test_compose_summary_reports_failed_company_profile_as_data_gap(self):
        ...
        self.assertIn("research-profile(company_profile) 失败: profile report missing", summary)
    ...
    def test_write_outputs_prompt_plan_creates_plan_markdown_and_json(self):
        ...
        self.assertIn("使用 invest-flow:research-fundamentals 分析 MRVL", plan)
    ...
    def test_compose_summary_subreport_index_lists_all_five_dimensions(self):
        from investflow_pipeline.composer import compose_summary
        from investflow_pipeline.models import AnalysisStatus, Handoff, StageResult
        ...
        stages = [
            ...
            StageResult("research-reflexivity", "reflexivity", AnalysisStatus.PENDING),
            StageResult("research-reportify", "reportify", AnalysisStatus.PENDING),
        ]
        ...
        self.assertIn(
            "- **research-institutional**：未生成子报告链接（failed）",
            summary,
        )
        self.assertIn("- **research-reflexivity**：未生成子报告链接（pending）", summary)
        self.assertIn("- **research-reportify**：未生成子报告链接（pending）", summary)
...
class RunnerTests(unittest.TestCase):
    ...
    def test_analyze_stock_sync_writes_prompt_plan_and_json(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.stage_results[0].skill_name, "research-profile")
...
class OrchestratorCompatibilityTests(unittest.TestCase):
    ...
    def test_orchestrator_cli_prints_prompt_plan_and_returns_zero(self):
        ...
        self.assertIn("使用 invest-flow:research-profile 分析 MRVL / Marvell Technology", output)
        self.assertIn("使用 invest-flow:research-fundamentals 分析 MRVL", output)

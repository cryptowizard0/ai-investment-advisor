...
本 skill 编排 chain-alpha 四步工作流：行业定义与增长初筛、拆解产业链并找供需错位（同属 Step 1）-> 分析关键环节垄断格局（Step 2）-> 验证公司并定档位（Step 3）-> 形成入场计划（Step 4）。它不引入新的分析逻辑，只负责调度四个步骤 skill、传递交接数据、执行漏斗纪律和生成最终汇总。

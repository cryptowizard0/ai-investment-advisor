...
2. **选尺子 PE or PS**：默认 PE；四个 PE 失真条件（5 年 TTM PE 中位数 >100、当前 TTM PE 为负、5 年内连续 ≥4 季净利润为负、重要参考点 TTM PE 为负）任一满足改用 PS。选定尺子后立即做两项检查：**警戒线**（当前 TTM PE >100 倍或 TTM PS >40 倍 → 重点提示并停止，不做后续步骤）；**历史数据不足**（上市较晚、分位样本覆盖不足 5 年 → 重点提示并全程降置信度，不停止）。
...
- **警戒线检查**：当前 TTM PE >100 倍或 TTM PS >40 倍 → 重点提示并停止（脚本自动执行；阈值可用 `--pe-alert`/`--ps-alert` 调整但须说明理由）。
...
- 警戒线（PE >100 倍 / PS >40 倍）触发即停止，不得以成长性叙事豁免；报告须给出"先降敞口、回线内再复评"的处置提示。

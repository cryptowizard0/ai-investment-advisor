...
5. **建仓计划**：**仓位上限 = 回撤预算 ÷ 潜在风险**。回撤预算是总仓位回撤百分比——单只股票最多允许给整个账户带来多少回撤。不给单一默认值，报告按 **2% / 5% / 10% / 20% / 30% / 50% / 70%** 逐档列出对应仓位上限（`--drawdown-budget` 指定主档，默认 2%，表中加粗并用于结论）。例：回撤预算 2% ÷ 潜在风险 40% = 仓位上限 5%。折扣叠乘：chain-alpha 档位（金池子全额 / 通过 ×0.5 / 待验证、剔除不给仓位）、弹性标的（占比 20-40%）×0.5、历史数据不足 ×0.5；**警戒线触发时新建仓归零**（各档公式值仅作回线内复评参考）。
...
### 6) 解读并补齐
- 填写报告"解读"区（必答三条见 methodology 第 7 节）：当前定位与形态、风险承受判断、分位带失效条件。
- 补齐数据来源表（来源 + 日期 + 口径）。
...
- **上游 `chain-alpha-verification`（③）**：只输出档位与弹性标记，不定仓位；"通过及以上"标的进入本 skill，档位通过 `--grade`、弹性通过 `--elastic` 带入并转成仓位折扣。
...
### references/
- `methodology.md`: 类型闸门细则与失效机理、选尺子四条件口径、数据口径、分位计算方法、风险公式、建仓计划公式与折扣规则、解读要求。
- `report-template.md`: 标准中文 Markdown 报告模板（结论 / 闸门 / 尺子 / 分位风险表 / 读数 / 建仓计划 / 解读 / 数据来源）。

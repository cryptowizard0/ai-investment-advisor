...
本 skill 是 chain-alpha 工作流的第二步：找壁垒、找垄断、排除低端竞争，并确认垄断优势是否能转化为可持续利润增速。输入一个供需错位环节（通常来自 `chain-alpha-mismatch`），输出该环节内 ≤10 家通过硬门槛的候选公司，并标注上市地与可投性。
...
- `使用 invest-flow:chain-alpha-monopoly 筛选 <环节>`

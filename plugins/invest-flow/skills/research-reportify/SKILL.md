...
description: "个股分析报告生成器（含买方研究风格深度）。基于固定方法论（事实层到解释层到决策层）对股票进行结构化分析，并按八段式模板输出中文 Markdown 报告，决策层带三情景估值、可证伪投资假设、催化剂时间表和跟踪指标 Dashboard。适用于：(1) 用户要求分析单个股票或生成正式/买方风格个股报告, (2) 需要统一格式、可比较、可复盘、有证据链的投研报告, (3) 需要把财务/技术/风险信息整合成可执行、可跟踪的投资建议。支持输出到 ./output/research/ 并自动处理重名文件。"
---

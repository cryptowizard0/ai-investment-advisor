...
| `company-valuation-risk` | chain-alpha 第四步：类型闸门 → PE/PS 选尺（含警戒线）→ 5 年 TTM 分位带 → 潜在风险两腿取大 → 增速消化核对（成长模型合理 PE，防机械读分位）→ 建仓计划（仓位上限 = 回撤预算 ÷ 潜在风险，档位/弹性折扣叠乘，警戒线触发新仓归零）。 | verification 给出档位后，需要买点判断和仓位上限。（同时列在第二类。） |

...
- `chain-alpha-mismatch-discovery` - chain-alpha step 1: full industry-chain panorama list plus supply-demand mismatch link discovery with hard evidence gates, 25-point mismatch scoring including a reverse-scored pricing-in dimension, and a supply-response clock that reads expansion evidence both ways
...
- `chain-alpha-delivery-tracking` - chain-alpha follow-on: forward-looking revenue-delivery tracking for 待验证 candidates via a 5-gate validation ladder (order->capacity->ramp->revenue->profit) with per-gate delivery windows and timeout downgrades, growth/attribution/dynamic-valuation (PE&PS dual-track) engines, structure sentinels (competitor capacity, second-sourcing, customer in-housing, demand-side capex), and symmetric grade up/down that feeds back into chain-alpha

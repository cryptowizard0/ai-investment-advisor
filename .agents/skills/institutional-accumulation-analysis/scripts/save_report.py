...
Output:
    ./output/institutional-accumulation-analysis/机构操作分析-{YYYYMMDD}-{TICKER}.md
"""
...
def get_output_directory():
    ...
    Returns:
        Path: Output directory at ./output/institutional-accumulation-analysis/
    """
    ...
    if "scripts" in Path.cwd().name or ".agents" in str(Path.cwd()) or ".claude" in str(Path.cwd()):
        ...
        if found_root:
            output_dir = project_root / "output" / "institutional-accumulation-analysis"
...
def generate_filename(ticker):
    ...
    return f"机构操作分析-{date_str}-{ticker.upper()}.md"
...
def main():
    ...
    parser.add_argument('--output-dir', '-o', help='Custom output directory (default: ./output/institutional-accumulation-analysis/)')
